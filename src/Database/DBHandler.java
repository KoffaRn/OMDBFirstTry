...
public class DBHandler {
    ...
    public boolean addMovie(DBMovie dbMovie) {
        try {
            ...
            ps.setString(3, dbMovie.getYear().toString());
    ...
    public DBMovie[] getMoviesByTitle(String qtitle) {
        try {
            ...
            while(rs.next()) {
                ...
                dbMovie.setGenres(getGenresByImdbId(rs.getString("imdbID")));
                dbMovie.setActors(getActorsByImdbId(rs.getString("imdbID")));
                dbMovie.setRatings(getRatingsByImdbId(rs.getString("imdbID")));
                dbMovies[i] = dbMovie;
                i++;
            }
            return dbMovies;
    ...
    public DBMovie getMovieByImdbId(String imdbId) {
        try {
            ...
            if(rs.next()) {
                ...
                while(rs2.next()) {
                    dbMovie.addGenre(new DBGenre(rs2.getString("imdbID"), rs2.getString("genre")));
                }
                PreparedStatement ps3 = conn.prepareStatement("SELECT * FROM actors WHERE imdbID = ?");
                ps3.setString(1, imdbId);
                ResultSet rs3 = ps3.executeQuery();
                while(rs3.next()) {
                    dbMovie.addActor(new DBActor(rs3.getString("imdbID"), rs3.getString("name"), valueOf(rs3.getString("role"))));
                }
                PreparedStatement ps4 = conn.prepareStatement("SELECT * FROM ratings WHERE imdbID = ?");
                ps4.setString(1, imdbId);
                ResultSet rs4 = ps4.executeQuery();
                while(rs4.next()) {
                    dbMovie.addRating(new DBRating(rs4.getString("imdbID"), rs4.getString("source"), rs4.getString("value")));
                }
                return dbMovie;
            } else {
                return null;
    ...
    public DBMovie[] getMoviesByActor(DBActor actor) {
        PreparedStatement ps = null;
        try {
            ps = conn.prepareStatement("SELECT * FROM actors WHERE name = ?");
